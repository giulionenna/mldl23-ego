...
#
	...
	def forward(self, input_source, input_target, beta, mu, is_train, reverse):
		...
		# === adversarial branch (frame-level) GSF===#
...
class ClassifierMidFusion(nn.Module):
	def __init__(self,device, num_class, baseline_type, frame_aggregation, modality, n_modalities,
				train_segments=5, val_segments=25,
				base_model='resnet101', path_pretrained='', new_length=None,
				before_softmax=True,
				dropout_i=0.5, dropout_v=0.5, use_bn='none', ens_DA='none',
				...
				use_attn='TransAttn', n_attn=1, use_attn_frame='none',
				share_params='Y'):
		super(ClassifierMidFusion, self).__init__()
		self.device = device
		self.modality = modality
	...
	def domainAlign(self, input_S, input_T, is_train, name_layer, alpha, num_segments, dim):
		input_S = input_S.reshape((-1, dim, num_segments) + input_S.size()[-1:])  # reshape based on the segments (e.g. 80 x 512 --> 16 x 1 x 5 x 512)
		input_T = input_T.reshape((-1, dim, num_segments) + input_T.size()[-1:])  # reshape based on the segments
		...
		# adaptive BN
		input_source = input_source.reshape((-1, ) + input_source.size()[-1:]) # reshape to feed BN (e.g. 16 x 1 x 5 x 512 --> 80 x 512)
		input_target = input_target.reshape((-1, ) + input_target.size()[-1:])
		...
		# reshape for frame-level features
		if name_layer == 'shared' or name_layer == 'trn_sum':
			input_source_bn = input_source_bn.reshape((-1,) + input_source_bn.size()[-1:])  # (e.g. 16 x 1 x 5 x 512 --> 80 x 512)
			input_target_bn = input_target_bn.reshape((-1,) + input_target_bn.size()[-1:])
		elif name_layer == 'trn':
			input_source_bn = input_source_bn.reshape((-1, num_segments) + input_source_bn.size()[-1:])  # (e.g. 16 x 1 x 5 x 512 --> 80 x 512)
			input_target_bn = input_target_bn.reshape((-1, num_segments) + input_target_bn.size()[-1:])

		return input_source_bn, input_target_bn

	def gsf(self,m,input_data_source,input_data_target,batch_source,batch_target, num_segments,is_train):
		feat_base_source = input_data_source.reshape(-1, input_data_source.size()[-1]) # e.g. 256 x 25 x 2048 --> 6400 x 2048
		feat_base_target = input_data_target.reshape(-1, input_data_target.size()[-1])  # e.g. 256 x 25 x 2048 --> 6400 x 2048
		...
		if(m ==0):
			...
			feat_all_target  = []
			# feat_fc = self.dropout_i(feat_fc)
			feat_all_source.append(feat_fc_source.reshape((batch_source, num_segments) + feat_fc_source.size()[-1:])) # reshape ==> 1st dim is the batch size
			feat_all_target.append(feat_fc_target.reshape((batch_target, num_segments) + feat_fc_target.size()[-1:]))
			...
			feat_fc_source = self.fc_feature_shared_source_m1(feat_base_source)
			feat_fc_target = self.fc_feature_shared_target_m1(feat_base_target) if self.share_params == 'N' else self.fc_feature_shared_source(feat_base_target)
			...
			if self.add_fc > 1: 
				feat_fc_source = self.fc_feature_shared_2_source_m1(feat_fc_source)
				feat_fc_target = self.fc_feature_shared_2_target_m1(feat_fc_target) if self.share_params == 'N' else self.fc_feature_shared_2_source(feat_fc_target)
			...
			if self.add_fc > 2: 
				feat_fc_source = self.fc_feature_shared_3_source_m1(feat_fc_source)
				feat_fc_target = self.fc_feature_shared_3_target_m1(feat_fc_target) if self.share_params == 'N' else self.fc_feature_shared_3_source(feat_fc_target)
			...
			feat_fc_source = self.fc_feature_shared_source_m2(feat_base_source)
			feat_fc_target = self.fc_feature_shared_target_m2(feat_base_target) if self.share_params == 'N' else self.fc_feature_shared_source(feat_base_target)
			...
			if self.add_fc > 1: 
				feat_fc_source = self.fc_feature_shared_2_source_m2(feat_fc_source)
				feat_fc_target = self.fc_feature_shared_2_target_m2(feat_fc_target) if self.share_params == 'N' else self.fc_feature_shared_2_source(feat_fc_target)
		...
		return feat_all_source,feat_all_target
	def forward(self, input_source, input_target, beta, mu, is_train, reverse):
		batch_source = input_source.size()[0]
		batch_target = input_target.size()[0]

		n_modalities = (input_target.size()[1]//5) #m = 1 -> 5, m=2 -> 10, m=3 ->15
...
class ClassifierMidFusion2(nn.Module):
	def __init__(self,device, num_class, baseline_type, frame_aggregation, modality, n_modalities,
				train_segments=5, val_segments=25,
				base_model='resnet101', path_pretrained='', new_length=None,
				before_softmax=True,
	...
	def _prepare_DA(self, num_class, base_model): # convert the model to DA framework
		...
		if self.n_modalities >= 2:
			self.fc_feature_shared_source_m1 = nn.Linear(self.feature_dim, feat_shared_dim)
			normal_(self.fc_feature_shared_source_m1.weight, 0, std)
			constant_(self.fc_feature_shared_source_m1.bias, 0)

			if self.add_fc > 1:
				self.fc_feature_shared_2_source_m1 = nn.Linear(feat_shared_dim, feat_shared_dim)
				normal_(self.fc_feature_shared_2_source_m1.weight, 0, std)
				constant_(self.fc_feature_shared_2_source_m1.bias, 0)

			if self.add_fc > 2:
				self.fc_feature_shared_3_source_m1 = nn.Linear(feat_shared_dim, feat_shared_dim)
				normal_(self.fc_feature_shared_3_source_m1.weight, 0, std)
				constant_(self.fc_feature_shared_3_source_m1.bias, 0)
		if self.n_modalities >= 3:
			self.fc_feature_shared_source_m2 = nn.Linear(self.feature_dim, feat_shared_dim)
			normal_(self.fc_feature_shared_source_m2.weight, 0, std)
			constant_(self.fc_feature_shared_source_m2.bias, 0)

			if self.add_fc > 1:
				self.fc_feature_shared_2_source_m2 = nn.Linear(feat_shared_dim, feat_shared_dim)
				normal_(self.fc_feature_shared_2_source_m2.weight, 0, std)
				constant_(self.fc_feature_shared_2_source_m2.bias, 0)

			if self.add_fc > 2:
				self.fc_feature_shared_3_source_m2 = nn.Linear(feat_shared_dim, feat_shared_dim)
				normal_(self.fc_feature_shared_3_source_m2.weight, 0, std)
				constant_(self.fc_feature_shared_3_source_m2.bias, 0)
		# 2. frame-level feature layers************************************************************
		self.fc_feature_source = nn.Linear(feat_shared_dim, feat_frame_dim)
		normal_(self.fc_feature_source.weight, 0, std)
		constant_(self.fc_feature_source.bias, 0)
